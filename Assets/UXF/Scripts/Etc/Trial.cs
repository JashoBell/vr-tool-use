...
namespace UXF
{
    ...
    public class Trial : ISettingsContainer, IDataAssociatable
    {
        ...
        public void Begin()
        {
            ...
            {
                //reaching trials (where fastUpdate is used) will invoke StartRecording at a specific point in the trial.
                if(tracker.updateType != TrackerUpdateType.fastUpdate)
                {
                    try
                    {
        ...
        public void End()
        {
            ...
            foreach (Tracker tracker in session.trackedObjects)
            {
                //These tracker types call StopRecording and SaveDataTable within the task's Coroutine
                if(tracker.updateType != TrackerUpdateType.fastUpdate || tracker.updateType != TrackerUpdateType.buttonInput)
                {
                    try
                    {
        ...
        public void SaveDataTable(UXFDataTable table, string dataName, UXFDataType dataType = UXFDataType.OtherTrialData)
        {
            ...
            {
                //If a later start trial or block is chosen, make sure the trial number appended to the data table's file name is consistent with beginning later in the block/experiment.
                string location;
                ...
                if(startBlock > 1)
                {
                    ...
                    trialIncrement += startTrial - 1;
                }
                
                location = dataHandler.HandleDataTable(table, session.experimentName, session.ppid, session.number, dataName, dataType, number + trialIncrement);
